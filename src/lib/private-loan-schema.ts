...
export const privateLoanFieldSchema: PrivateLoanFieldSchema[] = [
  ...
  },
  {
    ...
    groupCode: "F",
    groupLabel: "风险筛查事实",
    label: "是否存在现金交付或利息争议风险",
    prompt: "是否存在大额现金交付、以息抵本、利息约定争议等需要重点核验的风险事实？",
    inputType: "single-select",
    options: [
  ...
  {
    id: "risk-relationship-mixing",
    groupCode: "F",
    groupLabel: "风险筛查事实",
    label: "是否存在其他法律关系混同风险",
    prompt: "是否存在货款、投资、代付、往来款等其他法律关系与借款事实交叉混同？",
    inputType: "single-select",
    options: [
      { value: "risk_none", label: "未见关系混同风险" },
      { value: "risk_relationship_mixing", label: "存在其他法律关系混同或高风险往来" },
    ],
    ruleTag: "RELATIONSHIP_MIXING_RISK",
    required: true,
    sourceType: "材料抽取",
    status: "待核实",
    sourcePosition: "交易背景陈述 / 对账记录 / 往来凭证",
    evidenceStrength: "中",
    helpText: "关系混同风险用于识别转账性质是否可能被其他交易背景覆盖。",
    isKeyField: true,
    value: "risk_none",
  },
];
